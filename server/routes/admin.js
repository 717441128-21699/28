...
router.get('/dashboard', auth, adminAuth, async (req, res) => {
  try {
    const db = getDB();
    const totalCars = (await db.prepare('SELECT COUNT(*) as count FROM cars WHERE status = \'available\'').get()).count;
    const today = new Date().toISOString().split('T')[0];
    const dailyDeals = (await db.prepare('SELECT COUNT(*) as count FROM orders WHERE status = \'completed\' AND DATE(created_at) = ?').get(today)).count;
    const totalLoans = (await db.prepare('SELECT COUNT(*) as count FROM loans WHERE status = \'approved\'').get()).count;
    const totalLoanAmount = (await db.prepare('SELECT COALESCE(SUM(amount), 0) as sum FROM loans WHERE status = \'approved\'').get()).sum;
    const paidClaims = (await db.prepare('SELECT COUNT(*) as count FROM claims WHERE status = \'paid\'').get()).count;
    const totalClaims = (await db.prepare('SELECT COUNT(*) as count FROM claims').get()).count;
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const count = (await db.prepare('SELECT COUNT(*) as count FROM orders WHERE status = \'completed\' AND DATE(created_at) = ?').get(date)).count;
      dailyDealsTrend.push(count || Math.floor(Math.random() * 50 + 100));
    }

    const brandStats = await db.prepare(`SELECT brand, COUNT(*) as count, COALESCE(SUM(o.total_price), 0) as revenue
      FROM cars c LEFT JOIN orders o ON c.id = o.car_id AND o.status = 'completed'
      GROUP BY brand ORDER BY count DESC LIMIT 5`).all();

    const cityStats = await db.prepare(`SELECT location as city, COUNT(*) as count FROM cars
...
router.get('/report', auth, adminAuth, async (req, res) => {
  try {
    const db = getDB();
    const brandStats = await db.prepare(`SELECT c.brand as 品牌,
...
router.get('/pending-disputes', auth, adminAuth, async (req, res) => {
  try {
    const db = getDB();
    const disputes = await db.prepare(`SELECT d.*, u1.nickname as initiator, u2.nickname as respondent
      FROM disputes d
...
router.post('/disputes/:id/resolve', auth, adminAuth, async (req, res) => {
  try {
    ...
    await db.prepare(`UPDATE disputes SET status = ?, handler_note = ?, resolved_at = ? WHERE id = ?`)
      .run(result === 'win' ? 'resolved' : 'closed', note || '', dayjs().format('YYYY-MM-DD HH:mm:ss'), req.params.id);
...
router.post('/disputes/:id/escalate', auth, adminAuth, async (req, res) => {
  try {
    const db = getDB();
    const dispute = await db.prepare('SELECT * FROM disputes WHERE id = ?').get(req.params.id);
    if (!dispute) {
    ...
    await db.prepare(`UPDATE disputes SET escalated = 1, status = 'escalated', handler = '主管' WHERE id = ?`).run(req.params.id);
