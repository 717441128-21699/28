...
router.post('/', auth, (req, res) => {
  ...
  const id = uuidv4();
  db.prepare('INSERT INTO appointments (id, car_id, buyer_id, seller_id, time, location) VALUES (?, ?, ?, ?, ?, ?)')
    .run(id, carId, req.user.id, car.seller_id, time, location);
  sendMessage(car.seller_id, 'appointment', '新的看车预约',
    `您的"${car.title}"收到新的看车预约，请及时处理`, id);
  sendMessage(req.user.id, 'appointment', '预约提交成功',
...
router.get('/', auth, (req, res) => {
  ...
  const appointments = db.prepare(sql).all(req.user.id).map(a => ({
    ...a,
...
router.post('/:id/confirm', auth, (req, res) => {
  ...
  sendMessage(appt.buyer_id, 'appointment',
...
router.post('/:id/intent', auth, (req, res) => {
  const db = getDB();
  const appt = db.prepare('SELECT * FROM appointments WHERE id = ?').get(req.params.id);
  if (!appt) {
    return res.json(fail('预约不存在'));
  ...
  db.prepare('UPDATE appointments SET buyer_intent = 1, status = \'completed\' WHERE id = ?').run(req.params.id);
  sendMessage(appt.seller_id, 'transaction', '买家提交购买意向',
...
router.post('/order', auth, (req, res) => {
  ...
  const id = uuidv4();
  db.prepare(`INSERT INTO orders (id, car_id, buyer_id, seller_id, deposit, total_price, commission, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, 'deposit_paid')`).run(id, carId, req.user.id, car.seller_id, deposit, totalPrice, commission);
  db.prepare('UPDATE cars SET status = \'reserved\' WHERE id = ?').run(carId);
  addExp(req.user.id, 200, `支付定金锁定车辆：${car.title}`);
  sendMessage(req.user.id, 'transaction', '定金支付成功',
    `您已成功锁定"${car.title}"，请按预约时间前往过户`, id);
  sendMessage(car.seller_id, 'transaction', '车辆已被锁定',
...
router.get('/orders', auth, (req, res) => {
  ...
  const orders = db.prepare(sql).all(req.user.id).map(o => ({
    ...o,
...
router.get('/orders/:id', auth, (req, res) => {
  const db = getDB();
  const order = db.prepare(`SELECT o.*, c.title as car_title, c.images as car_images, c.location as car_location,
    u1.nickname as buyer_name, u2.nickname as seller_name
    FROM orders o
    LEFT JOIN cars c ON o.car_id = c.id
...
router.post('/orders/:id/sign', auth, (req, res) => {
  const db = getDB();
  ...
  db.prepare('UPDATE orders SET contract_signed = 1, status = \'appointment\' WHERE id = ?').run(req.params.id);
  res.json(success(null, '电子合同已签署'));
});

router.post('/transfer/confirm', auth, (req, res) => {
  const db = getDB();
  ...
  db.prepare('UPDATE orders SET transfer_confirmed = transfer_confirmed + 1, updated_at = datetime(\'now\', \'localtime\') WHERE id = ?').run(orderId);
  const updated = db.prepare('SELECT * FROM orders WHERE id = ?').get(orderId);
  if (updated.transfer_confirmed >= 2) {
    db.prepare('UPDATE orders SET status = \'completed\', updated_at = datetime(\'now\', \'localtime\') WHERE id = ?').run(orderId);
    db.prepare('UPDATE cars SET status = \'sold\' WHERE id = ?').get(order.car_id);
    db.prepare('UPDATE users SET total_deals = total_deals + 1 WHERE id IN (?, ?)').run(order.buyer_id, order.seller_id);
    db.prepare('UPDATE users SET total_spent = total_spent + ? WHERE id = ?').run(order.total_price, order.buyer_id);
    addExp(order.buyer_id, 500, `成功购车`);
    addExp(order.seller_id, 500, `成功售车`);
    sendMessage(order.buyer_id, 'transaction', '交易完成', '恭喜您完成购车交易！', orderId);
    sendMessage(order.seller_id, 'transaction', '交易完成', '恭喜您完成售车交易！尾款将在24小时内到账', orderId);
  }
