...
router.post('/', auth, (req, res) => {
  ...
  const id = uuidv4();
  db.prepare(`INSERT INTO disputes (id, order_id, initiator_id, respondent_id, reason, evidence, handler, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, 'reviewing')`).run(
    id, orderId, req.user.id, respondent, reason,
    evidence ? JSON.stringify(evidence) : null,
    '客服专员' + (100 + Math.floor(Math.random() * 900))
  );
  sendMessage(respondent, 'dispute', '收到纠纷通知',
    `关于订单${orderId}的纠纷已被发起，请及时配合处理`, id);
  sendMessage(req.user.id, 'dispute', '纠纷已提交',
...
router.get('/', auth, (req, res) => {
  ...
  const disputes = db.prepare(sql).all(...params).map(d => ({
    ...d,
...
router.post('/:id/evidence', auth, (req, res) => {
  ...
  db.prepare('UPDATE disputes SET evidence = ? WHERE id = ?').run(JSON.stringify(merged), req.params.id);
