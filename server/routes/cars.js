...
router.post('/', auth, async (req, res) => {
  try {
    ...
    await db.prepare(`INSERT INTO cars (
    ...
    const brandRow = await db.prepare('SELECT id FROM brands WHERE name = ?').get(brand);
    if (brandRow) {
      await db.prepare('UPDATE brands SET count = count + 1 WHERE name = ?').run(brand);
    }
...
router.get('/', async (req, res) => {
  try {
    ...
    const cars = (await db.prepare(sql).all(...params)).map(car => ({
      ...car,
...
router.get('/brands', async (req, res) => {
  try {
    const db = getDB();
    const brands = await db.prepare('SELECT * FROM brands ORDER BY count DESC LIMIT 12').all();
...
router.get('/:id', async (req, res) => {
  try {
    const db = getDB();
    const car = await db.prepare('SELECT c.*, u.nickname as seller_name, u.avatar as seller_avatar, u.credit_score as seller_credit, u.total_deals as seller_deals FROM cars c LEFT JOIN users u ON c.seller_id = u.id WHERE c.id = ?').get(req.params.id);
    if (!car) {
...
router.get('/user/mine', auth, async (req, res) => {
  try {
    const db = getDB();
    const cars = (await db.prepare('SELECT * FROM cars WHERE seller_id = ? ORDER BY created_at DESC').all(req.user.id)).map(car => ({
      ...car,
      images: JSON.parse(car.images || '[]'),
      tags: car.tags ? JSON.parse(car.tags) : []
    }));
